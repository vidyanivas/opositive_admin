...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM `treatment` WHERE `id` = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($category_id) || empty($about) || empty($address) || empty($city) || empty($country)) {
        ...
        if (empty($error)) {
            ...
            $stmt = $conn->prepare("UPDATE `treatment` SET `name` = ?, `category_id` = ?, `slug` = ?, `image` = ?, `about` = ?, `address` = ?, `city` = ?, `country` = ?, `status` = ? WHERE `id` = ?");
