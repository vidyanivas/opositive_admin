...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($content)) {
        ...
        if (empty($error)) {
            ...
            $stmt = $conn->prepare("INSERT INTO blogs (`title`, `content`, `slug`, `status`, `image`) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("sssis", $title, $content, $slug, $status, $image_path);
            $stmt->execute();
            // Set the success message and redirect to the doctors page
			$_SESSION['success_message'] = "Blogs added successfully!";
                    ...
                    <form name="add_blog" method="POST" enctype="multipart/form-data">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label>Content</label>
                                    <textarea name="content" class="form-control" placeholder="Blog Content" required></textarea>
                                </div>
                            </div>
							<div class="col-md-12">
                                <div class="form-group">
