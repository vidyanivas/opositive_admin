...
// Ensure the blog ID is passed in the URL
if (isset($_GET['id']) && !empty($_GET['id'])) {
    $blog_id = $_GET['id'];

    // Fetch the existing blog details from the database
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($content)) {
        ...
        if ($slug !== $blog['slug']) {
            $checkSlugQuery = "SELECT COUNT(*) FROM blogs WHERE slug = ?";
        ...
        // If there are no errors, proceed to update the blog in the database
        if (empty($error)) {
            // Prepare and execute the query to update the blog
            $updateQuery = "UPDATE blogs SET title = ?, content = ?, slug = ?, status = ?, image = ? WHERE id = ?";
            $stmt = $conn->prepare($updateQuery);
            $stmt->bind_param("sssssi", $title, $content, $slug, $status, $image_path_new, $blog_id);
            $stmt->execute();
            $stmt->close();

            // Set the success message and redirect to the blogs page
            $_SESSION['success_message'] = "Blog updated successfully!";
            header("Location: blogs.php?s=success");
            exit();
        }
    }
}
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label>Title</label>
                                    <input type="text" name="title" class="form-control" value="<?php echo htmlspecialchars($title); ?>" placeholder="Blog Title" required>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label>Content</label>
                                    <textarea name="content" class="form-control" placeholder="Blog Content" required><?php echo htmlspecialchars($content); ?></textarea>
                                </div>
                            </div>
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <div class="form-check form-switch">
                                        ...
                                        <label class="form-check-label" for="flexSwitchCheckChecked">Blog Active</label>
                                    </div>
                                </div>
                            </div>
                        </div>
